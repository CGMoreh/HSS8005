...
#  Topic = c(paste("{{< ", paste0("var topic.w", 0:8), " >}}"), "{{< var topic.w8+ >}}"),
 Topic = c("{{< var topic.w0 >}}", 
           paste(paste0("<b>{{< ", paste0("var topic.w", 1:8), " >}}</b>"),
                "<br>",
                paste("<sub><em>{{< ", paste0("var subtitle.w", 1:8), " >}}</em></sub>")),
          "{{< var topic.w8+ >}}"),
  Info = c(NA, paste0("./materials/info/info_w0", 1:8), NA),
  Notes = c(NA, paste0("./materials/notes/notes_w0", 1:8), NA),
  Lecture = c(NA, paste0("./materials/slides-frame/slides-frame_w0", 1:8), NA),
...
Slides = c(NA, paste0("./docs/materials/slides/w", 1:8), NA)
...
contents <- contents |>
  mutate(Date = format(as_date(Date), "%d %B"),
         ...
         Lecture = ifelse(is.na(Lecture), "",
                       ifelse(file.exists(glue('{Slides}.html')),
...
details <- tibble(
  Topic = c(paste(paste0("<b>[Week ", 1:8, "</b>]", glue('({Info}.qmd)')), 
                  "<br>", 
                  paste0("<b>{{< ", paste0("var topic.w", 1:8), " >}}</b>"),
                  "<br>",
                  paste("<em>{{< ", paste0("var subtitle.w", 1:8), " >}}</em>"))
            ),
...
tbl_details <- details |>
  kbl(escap = FALSE) |>
  kable_styling() |>
  column_spec(1, width = "30%", bold = F, extra_css = "vertical-align:top;") |>  
  column_spec(2, width = "70%", extra_css = "font-size: 16px; text-align: justify; text-justify: inter-word") 
